
import { Event, Video } from "../interfaces/class";




export const ARKIV: Video[] = [
    // YOUTUBE
    {
        name: "Johan Nissen, BMX - FLD ride",
        desc: `Johan: This footage is from FLD skatepark in Copenhagen - filmed during recovery of my recent ankle injuries (both left and right). Therefore it is kept close to the ground and tech, in what i would call Denmarks cleanest skatepark. Have a look.`,
        videoUrl: "https://www.youtube-nocookie.com/embed/-MmLeyJYzHs",
        riders: ["Johan Nissen"],
        tags: ["københavn"],
        date: new Date("2018-10-23")
    },
    {
        name: "BMX Roadtrip Denmark 2020",
        desc: `Jonhan: One week through the country in the RV with Konstantin, Danni and Johan. Chill times in the cities of Denmark, which we don't visit too often`,
        videoUrl: "https://www.youtube-nocookie.com/embed/6NAShxcIgK0",
        riders: ["Johan Nissen", "Konstantin", "Danni"],
        tags: ["københavn"],
        date: new Date("2020-11-28")
    },
    {
        name: "BMXBUTIKKEN DET DANSKE HOLD - 2014",
        desc: `"En TBT fra en sommer med det meste af teamet filmet fra johans synspunkt. Super sommer vibes"`,
        videoUrl: "https://www.youtube-nocookie.com/embed/CHStnwOibQs",
        riders: ["Johan Nissen", "Marcus Diemar", "Danni", "Morten Pedersen", "Alexander Christensen", "Asger Duus", "Viggo Bro", "Kasper Jakobsen"],
        tags: ["københavn"],
        date: new Date("2014-04-28")
    },
    {
        name: "STREET JAM COPENHAGEN 2016",
        desc: `"Video by @ingus_r. BMX street jam in Copenhagen took place on 18th june 2016. Hosted by BMX Butikken's Johan Nissen and Asger Duus gathered more than 100 bmx riders from Denmark, Germany, Norway, Sweden, Latvia, Finland, Lithuania and even Greenland for a chance to win invitation to Roskilde festival BMX jam 2016 as well as other goods from BMX butikken, Vans, Red Bull, Carhartt and G-Shock. "`,
        videoUrl: "https://www.youtube-nocookie.com/embed/ZZR--_O7cBw",
        riders: [],
        tags: ["københavn", "Gade BMX", "Street jam"],
        date: new Date("2016-05-23")
    },
    {
        name: "dgp2",
        desc: `Fuld længde BMX video med alle de danske BMX-legender! Videoen er filmet rundt om i hele Danamrk og i Spanien`,
        videoUrl: "https://www.youtube-nocookie.com/embed/VoPrTZRZ8m0",
        riders: ["Dritaz Frederik", "Jesper Olsen", "Frede", "Kanan", "Mik", "Lasse søltoft", "Wagner", "Peter", "Anders", "Danni", "Thomas"],
        tags: ["dgp2", "Barcelona", "København", "Peter"],
        date: new Date("2013-03-02")
    },
    {
        name: "Niklas Svane Bentzen - All Access Skatepark",
        desc: `Edit fra All Access Skatepark i Randers. filmet og redigeret af Martin Paarup`,
        videoUrl: "https://www.youtube-nocookie.com/embed/ZtHA5BeoGxY",
        riders: ["Niklas Bentzen"],
        tags: ["Randers", "All Access"],
        date: new Date("2012-05-24")
    },
    {
        name: "Martin & Inuk Welcome to Rite.se",
        desc: `Martin Paarup og Inuk Siegstad`,
        videoUrl: "https://www.youtube-nocookie.com/embed/HGc3tr1i8Rw",
        riders: ["Niklas Bentzen"],
        tags: ["Randers"],
        date: new Date("2013-09-25")
    },
    {
        name: "One night in silkeborg - Martin Paarup",
        desc: `Gode grin og bangers. Filmet over en enkelt aften i Silkeborg skatepark`,
        videoUrl: "https://www.youtube-nocookie.com/embed/2BV3fWM8eWM",
        riders: ["Kasper Jakobsen", "Martin Paarup", "Inuk Siegstad", "Lasse Søltoft", "Jannik Nielsen", "Victor ottesen", "Jakob Rosendal"],
        tags: ["Randers"],
        date: new Date("2014-09-30")
    },
    {
        name: "Martin Paarup in Lisbon",
        desc: ` `,
        videoUrl: "https://www.youtube-nocookie.com/embed/iaJdGcsK-2E",
        riders: ["Martin Paarup"],
        tags: ["Lisabon"],
        date: new Date("2015-02-24")
    },
    {
        name: "Welcome to Soud Distrio - Martin Paarup",
        desc: `Filmet i Barcelona`,
        videoUrl: "https://www.youtube-nocookie.com/embed/x6aBT63sUjA",
        riders: ["Martin Paarup"],
        tags: ["Barcelona"],
        date: new Date("2015-02-24")
    },
    {
        name: "Rasmus Primby X Martin Paarup Iphone edit",
        desc: `Filmet i Randers`,
        videoUrl: "https://www.youtube-nocookie.com/embed/x6aBT63sUjA",
        riders: ["Martin Paarup", "Rasmus Primby", "Lasse Søltoft"],
        tags: ["Randers"],
        date: new Date("2016-06-08")
    },
    {
        name: "Martin Paarup in Amsterdam",
        desc: `Filmet af Victor Ottesen og Andreas Bank `,
        videoUrl: "https://www.youtube-nocookie.com/embed/kc_ba7GEVqI",
        riders: ["Martin Paarup"],
        tags: ["Amsterdam"],
        date: new Date("2017-11-22")
    },
    {
        name: "MARCUS DIEMAR - Welcome to Merrit",
        desc: `Merrit: "Proud to announce tech combo wizard Marcus Diemar as the latest addition to our International Team! Hailing from Copenhagen, Denmark Marcus is representing Merritt through the help of Centrano/SkatePro."`,
        videoUrl: "https://www.youtube-nocookie.com/embed/h9Scqkn7JV0",
        riders: ["Marcus Diemar"],
        tags: ["København"],
        date: new Date("2018-07-13")
    },
    {
        name: "MARCUS DIEMAR - Merrit",
        desc: `Merrit: "Merrit: "Marcus stacked up a few clips while Scott was out visiting Copenhagen. Filmed and edit Scot McMenamin."`,
        videoUrl: "https://www.youtube-nocookie.com/embed/2JM2Mv5Gxo4",
        riders: ["Marcus Diemar"],
        tags: ["København"],
        date: new Date("2019-08-02")
    },
    {
        name: "Marcus Diemar - Copenhagen Summer",
        desc: `Filmet of redigeret af Jonathan Paver Gregersen: "I spend this summer filming for this edit with Marcus Diemar in Copenhagen. We've had a lot of fun and been chilling in the summer sun, stacking clips - whenever the rain didn't ruin our day - or most of it."`,
        videoUrl: "https://www.youtube-nocookie.com/embed/rkKgfeop46E",
        riders: ["Marcus Diemar"],
        tags: ["København", "Jonathan Paver"],
        date: new Date("2017-11-13")
    },
    {
        name: "Aksel Breum - Iphone Mix Copenhagen 2018",
        desc: `Aksel Breum vil gerne give Steve Jobs og resten af hans hold et kæmpe tak for at designe Iphonen.`,
        videoUrl: "https://www.youtube-nocookie.com/embed/eeJh3rGDEbk",
        riders: ["Marcus Diemar", "Lito Abarca", "Asger Duus", "Patrick Dahlerup", "Aksel Breum", "Martin Jessin", "Lasse Pelle"],
        tags: ["København", "Jonathan Paver"],
        date: new Date("2018-12-21")
    },
    {
        name: "Copenhagen streetjam 2019 - Aksel Breums Highlights",
        desc: `Aksel Breum: "Rigtig drengerøvsjam"`,
        videoUrl: "https://www.youtube-nocookie.com/embed/1tfYBlW-680",
        riders: ["Marcus Diemar", "Lito Abarca", "Asger Duus", "Patrick Dahlerup", "Aksel Breum", "Martin Jessin", "Lasse Pelle", "silas", "Tejnø"],
        tags: ["København", "Gade BMX", "streetjam"],
        date: new Date("2019-08-07")
    },
    {
        name: "Copenhagen Street Jam Vol. 5 (2020 Covid Edition) by GadeBMX",
        desc: `Official Edit of Copenhagen Street Jam Vol.5 2020 by Lasse Christensen`,
        videoUrl: "https://www.youtube-nocookie.com/embed/MKJVtABWTQg",
        riders: ["Marcus Diemar", "Asger Duus", "Patrick Dahlerup", "Aksel Breum", "Martin Jessin", "Lasse Pelle", "Tejnø", "Alexander Kure", "Johan Nissen"],
        tags: ["København", "Gade BMX", "streetjam"],
        date: new Date("2020-12-03")
    },
    {
        name: "Copenhagen Street Jam 2018 - Anthony Panza",
        desc: `Klips fra Copenhagen Street Jam 2018 filmet af Anthony Panza`,
        videoUrl: "https://www.youtube-nocookie.com/embed/t16e9Kl_sh4",
        riders: ["Marcus Diemar", "Martin Jessin", "Tejnø", "Alexander Kure", "Johan Nissen"],
        tags: ["København", "Gade BMX", "streetjam", "Anthony Panza"],
        date: new Date("2018-07-01")
    },
    {
        name: "Copenhagen Street Jam 2019 - Billy Perry",
        desc: `Klips fra Copenhagen Street Jam 2019 filmet af Billy Perry`,
        videoUrl: "https://www.youtube-nocookie.com/embed/rimDo0oE1QM",
        riders: ["Marcus Diemar", "Martin Jessin", "Tejnø", "Alexander Kure", "Johan Nissen"],
        tags: ["København", "Gade BMX", "streetjam", "Billy Perry"],
        date: new Date("2019-07-02")
    },
    {
        name: "2 liter mælk, små cykler..",
        desc: `Filmet og redigeret af Lasse Rask`,
        videoUrl: "https://www.youtube-nocookie.com/embed/Fldc4a6lLWY",
        riders: ["Sebastian Tao Fog"],
        tags: ["tjele efterskole", "Lasse Rask"],
        date: new Date("2014-09-01")
    },



    // VIMEO
    {
        name: "Q Late Summer",
        desc: `Jonathan Gregersen: "Sensommer vibes fra Roskilde, med Q i hovedrollen og mig bag kameraet/ne. Nyd den! Vi har nydt at lave den !"`,
        videoUrl: "https://player.vimeo.com/video/106866903",
        riders: ["Rasmus Q"],
        tags: ["Roskilde", "Jonathan Gregersen"],
        date: new Date("2014-09-22")
    },
    {
        name: "Midnatssol session",
        desc: `En sen session i Roskildes hal12 af Jonathan Gregersen`,
        videoUrl: "https://player.vimeo.com/video/149632743",
        riders: ["Marcus Diemar", "Martin Jessin", "Johan Nissen"],
        tags: ["Roskilde", "Jonathan Gregersen"],
        date: new Date("2015-12-21")
    },
    {
        name: "Desværre",
        desc: `Længere video fra Roskilde af Jonathan Gregersen`,
        videoUrl: "https://player.vimeo.com/video/96436230",
        riders: ["Marcus Diemar", "Martin Jessin", "Rasmus Lynge", "Rasmus Q", "Zean Poulsen", "Jonathan Gregersen", "Gilbert Andersen", "Martin Lind"],
        tags: ["Roskilde", "Jonathan Gregersen"],
        date: new Date("2015-09-21")
    },
    {
        name: "#xmasjam13",
        desc: `Jonathan Gregersen: "En kort video, fra et super svedigt jam, fyldt med bmx, druk, søvnløshed og alt for lidt god musik."`,
        videoUrl: "https://player.vimeo.com/video/82240894",
        riders: ["Marcus Diemar", "Martin Jessin", "Rasmus Lynge", "Rasmus Q", , "Jonathan Gregersen", "Janick Nielsen", "Lasse Søltoft", "Viggo Bro"],
        tags: ["Aalborg", "Jonathan Gregersen", "x-mas"],
        date: new Date("2013-12-18")
    },
    {
        name: "X-mas jam 2008",
        desc: `Michael Larsen: "Lidt klip fra x-mas jam 2008 i Vejgaard skatehal."`,
        videoUrl: "https://player.vimeo.com/video/82240894",
        riders: ["Michael Larsen", "Morten Pedersen", "Niels Tanild", "Lasse Søltoft", , "Kim Møldrup", "John Brun", "Morten Larsen"],
        tags: ["Aalborg", "Michael Larsen", "x-mas"],
        date: new Date("2008-12-18")
    },
    {
        name: "HOLD FAST #1 - intro",
        desc: `Den første store video fra det gamle Hold Fast Crew. Find alle dele på http://holdfast1.blogspot.com/`,
        videoUrl: "https://player.vimeo.com/video/8485724",
        riders: ["Michael Larsen", "Morten Pedersen", "Morten Larsen", "Lasse Søltoft"],
        tags: ["Aalborg", "Michael Larsen", "x-mas"],
        date: new Date("2009-12-12")
    },
    {
        name: "BMXslisken i Minihuset",
        desc: `Session fra den nu lukkede skatepark "Minihuset" i Viborg.`,
        videoUrl: "https://player.vimeo.com/video/16724812",
        riders: ["Michael Larsen", "Morten Pedersen", "Morten Larsen", "Lasse Søltoft", "Peter Svendsen"],
        tags: ["Viborg", "Michael Larsen", "minihuset"],
        date: new Date("2010-11-11")
    },
    {
        name: "Hold Fast #2 del 1",
        desc: `Den anden store video fra det gamle Hold Fast Crew. Find alle dele på http://holdfast2.blogspot.com/`,
        videoUrl: "https://player.vimeo.com/video/18546948",
        riders: ["Michael Larsen", "Morten Pedersen", "Morten Larsen", "Lasse Søltoft", "Gregers Pedersen", "Lasse Bech", "Jakob Holm", "Jakob Larsen", "Paw Friis", "Niklas Svane"],
        tags: ["Aalborg", "Michael Larsen", "hold fast"],
        date: new Date("2010-12-11")
    },
    {
        name: "Hold Fast #3 del 1",
        desc: `En tredje store video fra det gamle Hold Fast Crew. Find alle dele på Michael Larsens Vimeo`,
        videoUrl: "https://player.vimeo.com/video/47368636",
        riders: ["Michael Larsen", "Morten Pedersen", "Morten Larsen", "Niklas Svane", "Mik Krogh", "Travis Johnston", "Jakob Larsen", "Konstantin Dragun", "Tilano Roser", "Emil Frandsen", "Tobias Öreteg", "Kim Møldrup", "Asger Duus", "Danni Lærkegaard"],
        tags: ["Aalborg", "Michael Larsen", "hold fast"],
        date: new Date("2011-12-11")
    },
    {
        name: "Xmas jam 2012",
        desc: `Klips fra x-mas jam 2012 i Vejgaard skatehal Aalborg.`,
        videoUrl: "https://player.vimeo.com/video/47368636",
        riders: ["Michael Larsen", "Morten Pedersen", "Morten Larsen", "Niklas Svane", "Oliver pless","Alexander Christensen", "Martin Paarup", "Inuk Siegstad", "Lasse Søltoft"],
        tags: ["Aalborg", "Michael Larsen", "hold fast"],
        date: new Date("2012-12-11")
    },
    {
        name: "Svendborg Cooltour Jam",
        desc: `Ledge action fra Svendborg Cooltour Jam`,
        videoUrl: "https://player.vimeo.com/video/59419803",
        riders: ["Michael Larsen", "Morten Larsen", "Xander", ],
        tags: ["Svendborg ", "Michael Larsen", "hold fast"],
        date: new Date("2013-02-11")
    },
]








export const VVK2020: Video[] = [
    {
        name: "8K BMX",
        desc: `8K BMX udgør BMX'ere fra adskilige lande, der til dagligt kører i Århus C og omegn. Det er første gang at drengene laver en video sammen, men vi på at se mange flere! God bedring til Nikolaj."`,
        videoUrl: "https://www.youtube-nocookie.com/embed/h_WoDKbrYYk",
        riders: ["Tim Swa", "Max Chrls", "Nikolaj Weber", "Šimon Čechura", "Henry Hjorth"],
        tags: ["Århus"],
        date: new Date("2021-03-31")
    },
    {
        name: "Tonny Kristensen",
        desc: "Tonny er fra den gamle BMX skole og kan stensikkert introducere nogle moves til den yngre generation. Tony er begyndt at køre igen efter en lille pause på over 30 år.",
        videoUrl: "https://www.youtube-nocookie.com/embed/vg3N1s-EbAQ",
        riders: ["Tonny Kristensen"],
        date: new Date("2021-03-31"),
        tags: ["strandby"]

    },
    {
        name: "Team Puthøne Freestyling",
        date: new Date("2021-03-21"),
        desc: "Drengene fra Roskilde har trodset vintervejret og udelukkende kørt udenfor. Hatten af for at sætte flere store tricks i våde betonparks uden at glide.",
        videoUrl: "https://www.youtube-nocookie.com/embed/HxO6nL2IRmk",
        tags: ["roskilde"],
        riders: ["Oliver Berry Østergaard", "Mikkel Jakobsen", "Claes Petersen"],
    },
    {
        name: "Mikkel bergfriedt Lykkegaard",
        date: new Date("2021-02-03"),
        desc: "En ung fyr fra Galten, på 17 år, som kører bmx i alt sin fritid og nyder hvert sekund han har fødderne på pedalerne!",
        videoUrl: "https://www.youtube-nocookie.com/embed/L2aR86RvI_k",
        riders: ["Mikkel bergfriedt Lykkegaard"],
        tags: ["galten"]
    },
    {
        name: "Victor Ottesen",
        date: new Date("2021-04-01"),
        desc: "Victor har en unik stil og kan altid overraske med tricks og comboer som du nok ikke havde kunne finde på selv. Han kører for Fiction Bikes og Stolen Bike Co igennem Centrano",
        videoUrl: "https://www.youtube-nocookie.com/embed/PPrMRn0sip4",
        tags: ["silkeborg"],
        riders: ["Victor Ottesen"]
    },
    {
        name: "Qillaq Nielsen",
        date: new Date("2021-04-01"),
        desc: "Qillaq er 22 år og har kørt bmx i de sidste 10 år. Han har tech comboer som ingen andre herhjemme og kan snurre hurtigere end en fuldt tunet beyblade!",
        videoUrl: "https://www.youtube-nocookie.com/embed/CnjXl5r3y28",
        tags: ["silkeborg"],
        riders: ["Qillaq Nielsen"]
    },
    {
        name: "Primby & Søltoft",
        riders: ["Rasmus Primby", "Lasse Søltoft"],
        date: new Date("2021-01-01"),
        desc: "To daddies på BMX cykler, direkte ud af BrianTown Randers, som begge er med til at inspirere den nye generation af BMX’ere på Tjele Efterskole. Lasse og Rasmus var tidligt ude og afleverede allerede deres færdige video til konkurencen d. 31. december.",
        videoUrl: "https://www.youtube-nocookie.com/embed/uP2ZzCAnVKw",
        tags: ["silkeborg"],

    },
    {
        name: "De 3 unge fra Silkebronx",
        riders: ["Sebastian Nielsen", "Malthe Winther", "Oliver Meyer"],
        tags: ["silkeborg"],
        date: new Date("2021-03-30"),
        desc: "3 unge drenge fra Silkeborg, som har kørt sammen i snart halvandet år. De er efter eget udsagn tosessed med sporten!",
        videoUrl: "https://www.youtube-nocookie.com/embed/MHEqL3_ADlM"
    },
    {
        name: "Benjamin & Oliver ",
        riders: ["Benjamin Kornbo", "Oliver Pless"],
        tags: ["randers"],
        date: new Date("2021-03-26"),
        desc: "Benjamin og Oliver fra Team Randers smider bangers i samtlige skateparks i og omkring Randers.",
        videoUrl: "https://www.youtube-nocookie.com/embed/GRxKv7kO9L8"
    },
    {
        name: "fknshtbmx",
        riders: ["Jeppe Klinkby", "Martin Skovgaard"],
        tags: ["aalborg"],
        date: new Date("2021-03-29"),
        desc: "Bicycle Moto Fuckin' Cross. Find dem på instagram under @fknshtbmx",
        videoUrl: "https://player.vimeo.com/video/530089724"
    },
    {
        name: "Tænke",
        riders: ["Martin Paarup", "Lasse Christensen", "Anders Madsen", "Marcus Tindahl"],
        tags: ["aalborg"],
        date: new Date("2021-03-30"),
        desc: "Team Tænke består af fire friske gutter fra Aalborg, som alle er super dygtige på deres cykler. De er i øjeblikket også i gang med at filme en hel BMX-DVD!",
        videoUrl: "https://www.youtube-nocookie.com/embed/xEA73_Wp4S4"
    },
    {
        name: "Stefan Nielsen",
        riders: ["Stefan Nielsen"],
        tags: ["aalborg"],
        date: new Date("2021-04-01"),
        desc: "Stefan er en old boy fra Øster Hornum. Selvom han deltager i Old Boy kategorien, har han masser af moderne tricks i ærmet.",
        videoUrl: "https://www.youtube-nocookie.com/embed/srZEshLSwfc"
    },
    {
        name: "Party Pøller",
        riders: ["Jakob Rosendal"],
        tags: ["glejbjerg"],
        date: new Date("2021-04-01"),
        desc: "Jakob har i starten af konkurrencen flyttet til København, så samtlige klips er filmet i hovedstaden.",
        videoUrl: "https://www.youtube-nocookie.com/embed/n_IoMIgYdZ4"
    },
    {
        name: "Team Daller",

        riders: ["Aksel", "Viggo", "Sergio", "Daller", "Diemar", "Silas"],
        tags: ["københavn"],
        date: new Date("2021-03-30"),
        desc: "DISCLAIMER FRA TEAM DALLER: \"Team dallers video indholder ulovlige rusmidler og voldige scener. Ingen af filmens scener er virkelige og udført af profesionelle. 16+.\" Freestyle BMX Danmark promovere ikke og står ikke indenfor adfæren i videoen.",
        videoUrl: "https://www.youtube-nocookie.com/embed/6EPVyPFHtbI"
    },
    {
        name: "Holmen dirt boys",
        riders: ["Asger Duus", "Lukas Keysell", "Andreas Houmann"],
        tags: ["holmen", "københavn"],
        date: new Date("2021-03-29"),
        desc: "Holmens fineste BMX'ere har slået sig sammen for at trodse den danske vinter og filme en fed dirt-video!",
        videoUrl: "https://www.youtube-nocookie.com/embed/juaZ850SRQQ"
    },
    {
        name: "Conrad Mattsson",
        riders: ["Conrad Mattsson"],
        date: new Date("2021-03-31"),
        desc: "Hvis du har været til Street Jam'et i København de seneste 3 år, har du højes sandsynligt mødt humørbomben Conrad. Skønt en alder på 13, har han allerede kørt BMX i tre år.",
        videoUrl: "https://www.youtube-nocookie.com/embed/PECoZpQD35g",
        tags: ["københavn"]
    },
    {
        name: "Dragun y Nissen",
        riders: ["Johan Nissen", "Konstantin Dragun"],
        tags: ["københavn"],
        date: new Date("2021-04-01"),
        desc: "Johan og Konstantin besluttede sig for at filme et edit med kun 5 dage til afleveringsfristen! Fra Johan: \"Manual Drengene Har filmet et par eksempler på, hvordan man kan lave tricks på et hjul. Det var grineren. Hyg jer, ik'\"",
        videoUrl: "https://www.youtube-nocookie.com/embed/bEeikbnMFtA"
    },
    {
        name: "Built Different",
        tags: ["københavn"],
        riders: ["Gilbert Andersen", "Lito Abarca", "Zean Poulsen", "Keno Röller-Siedenburg", "Kasper Matthiessen", "Tom Benson"],
        date: new Date("2021-03-31"),
        desc: "Et hold, samlet på kryds af Københavns bydele med en enkelt ting tilfælles, de er bygget anderledes. Med rødder spredt helt fra Amager til Sydafrika, Sverige, England og Tyskland, har holdet derfor en international approach og en dedikation til gaderne. Built Different holder det street og har valgt KUN at filme street clips til denne konkurrence.",
        videoUrl: "https://www.youtube-nocookie.com/embed/u0RljbgTsJA",
    },
    {
        name: "Lasse & Emil",
        riders: ["Lasse Kristensen", "Emil Rasmussen"],
        tags: ["Aalestrup"],
        date: new Date("2021-03-27"),
        desc: "Lasse og Emil går på Tjele efterskole til dagligt, men har under lockdown kørt sammen i deres egen DIY-park i Aalestrup.",
        videoUrl: "https://www.youtube-nocookie.com/embed/4JFwSAtsPzM"
    },
    {
        name: "Jonathan Langer",

        riders: ["Jonathan Langer"],
        date: new Date("2021-04-01"),
        desc: "Jonathan på 24 har kørt on and off de sidste 10 år, med Hørsholm skatepark som hans lokale park. Han befinder sig i Paris for tiden og hans edit er derfor herfra.",
        videoUrl: "https://www.youtube-nocookie.com/embed/06FeZHLEjFE",
        tags: ["Aalestrup"]
    },
    {
        name: "Rasmus Hansen ",

        riders: [
            "Rasmus Hansen"],

        date: new Date("2021-04-01"),
        desc: "Rasmus har kørt i 5 år og tager tit imod København for en god session.",
        videoUrl: "https://www.youtube-nocookie.com/embed/X4jrVYxdzyU",
        tags: ["jægerspris"]
    }

];

export const EVENTS: Event[] = [
    {
        title: "Vinter video konkurrencen 2020",
        info: "Første vinter video konkurrence som varede fra november 2020 til marts 2021",
        videos: VVK2020
    },
    {
        title: "Arkiv",
        info: "Gamle BMX-videoer",
        videos: ARKIV
    },
];

export const VIDEOS: Video[] = [

].concat(VVK2020)

